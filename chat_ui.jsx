...
function SingleSidebar({docInfo,setDocInfo,isOpen,onClose}){
  ...
  return React.createElement("div",{className:"slideR",style:{width:290,borderRight:`1px solid ${T.border}`,flexShrink:0,display:"flex",flexDirection:"column",background:T.bg,height:"100%"}},
      ...
      :React.createElement(React.Fragment,null,
        React.createElement("div",{onClick:()=>fileRef.current?.click(),style:{padding:"40px 20px",borderRadius:12,border:`2px dashed ${T.border}`,textAlign:"center",cursor:"pointer",transition:"all .15s"},
          ...
          :React.createElement(React.Fragment,null,
            ...
            React.createElement("p",{style:{fontSize:14,fontWeight:500,color:T.text2}},"Upload a file"),
            React.createElement("p",{style:{fontSize:12,color:T.text4,marginTop:4}},"PDF, CSV, or Excel"))))),
    ...
    React.createElement("input",{ref:fileRef,type:"file",accept:".pdf,.csv,.xlsx,.xls",onChange:handleUpload,style:{display:"none"}}));
}
...
function FolderSidebar({folders,setFolders,isOpen,onClose,refreshFolders}){
  ...
  const EXTS=[".pdf",".csv",".xlsx",".xls"];
  const onFiles=async e=>{const files=Array.from(e.target.files||[]);const fn=uploadTarget.current;if(!files.length||!fn)return;for(const file of files){const ext="."+file.name.split(".").pop().toLowerCase();if(!EXTS.includes(ext))continue;setIndexing({folder:fn,name:file.name});try{await API.folders.upload(fn,file);}catch(err){alert(`Failed: ${file.name} — ${err.message}`);}setIndexing(null);}await refreshFolders();e.target.value="";};
  if(!isOpen)return null;
  ...
  return React.createElement("div",{className:"slideR",style:{width:290,borderRight:`1px solid ${T.border}`,flexShrink:0,display:"flex",flexDirection:"column",background:T.bg,height:"100%"}},
    ...
    React.createElement("input",{ref:fileRef,type:"file",accept:".pdf,.csv,.xlsx,.xls",multiple:true,onChange:onFiles,style:{display:"none"}}));
}
...
function Welcome({mode,folders,docInfo,onSuggestion}){
  ...
  return React.createElement("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:"40px 24px",textAlign:"center"}},
    ...
    React.createElement("p",{className:"appear",style:{fontSize:13,color:T.text3,fontFamily:F.sans,marginTop:8,maxWidth:400,lineHeight:1.6,animationDelay:"100ms"}},mode==="single"?"Upload a PDF, CSV, or Excel file and ask anything. MCTS-powered reasoning finds the exact section.":"Ask anything across your documents. Auto-routes to the right folder using MCTS reasoning."),
    ...
    mode==="single"&&!docInfo&&React.createElement("p",{className:"appear",style:{fontSize:13,color:T.amber,fontFamily:F.sans,marginTop:20,animationDelay:"150ms"}},"Upload a file using the sidebar to get started."),
    ...
    mode==="folders"&&(!folders||folders.length===0)&&React.createElement("p",{className:"appear",style:{fontSize:13,color:T.amber,fontFamily:F.sans,marginTop:20,animationDelay:"150ms"}},"No folders yet — click Manage to create one and upload files."),
    ...
    React.createElement("div",{style:{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}},
      ...
      React.createElement("div",{style:{padding:"14px 20px 18px",flexShrink:0}},
        React.createElement("div",{style:{display:"flex",gap:8,alignItems:"flex-end",maxWidth:680,margin:"0 auto",padding:"5px 5px 5px 16px",borderRadius:14,background:T.surface,border:`1px solid ${T.border}`,transition:"border-color .15s,box-shadow .15s"},onFocus:e=>{e.currentTarget.style.borderColor=T.blue;e.currentTarget.style.boxShadow=`0 0 0 3px ${T.blueGlow}`},onBlur:e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.boxShadow="none"}},
          React.createElement("textarea",{value:input,onChange:e=>setInput(e.target.value),onKeyDown:e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}},placeholder:mode==="single"?(docInfo?"Ask about your document...":"Upload a file first..."):(folders.length?"Ask anything across your documents...":"Create a folder & upload files..."),disabled:isStreaming,rows:1,style:{flex:1,padding:"9px 0",background:"transparent",border:"none",outline:"none",color:T.text,fontSize:14,fontFamily:F.sans,resize:"none",lineHeight:1.5,maxHeight:100,minHeight:22},onInput:e=>{e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,100)+"px";}}),
